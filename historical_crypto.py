...
class HistoricalData (object):
  ...
  def retrieve_data(self):
   ...
   -------------------------------Returns---------------------------------------------  
    ...
    custom_url = "https://coinmarketcap.com/currencies/{0}/historical-data/?start={1}&end={2}".format(collect_ticker, self.start_date, self.end_date)
  
    try:
      dataset = pd.read_html(custom_url)
    except:
      raise WrongCoinCode("'{0}' is not available at CoinMarketCap.com. Check website for correct ticker name".format(collect_ticker))
